...
namespace db {
database::database()
    : _db(SQLite::Database("example.db3",
...
void database::refresh_and_seed_db() {
  ...
  _db.exec("PRAGMA writable_schema = 1;delete from sqlite_master where type in "
  ...
  std::string tables[NUM_TABLES] = {"Users",           "Students",
                                    "Instructors",     "Administrators",
                                    "Courses",         "Instructors_Courses",
                                    "Students_Courses"};
  ...
  for (int i = 0; i < NUM_TABLES; i++) {
    ...
    _db.exec(seed_query);
    std::cout << (i + 1) << " out of " << NUM_TABLES << " Table(s) done."
              << std::endl;
  }
