...
bool test_login_by_username_student_version () {
    ...
    lg.login (student.get_user_id (), "OmarReigns");

    // Check for current_user
    return lg.get_current_user () != nullptr &&
    lg.get_current_user ()->get_email () == student.get_email () &&
    lg.get_current_user ()->is_correct_password ("OmarReigns");
}

bool test_login_by_username_instructor_version () {
    // Create the student
    auth::instructor instructor ("Youssef Ihab"s, "Computer Science"s,
    "youssef.ihab@eui.com"s, "CoolnessNeverEnds"s);
